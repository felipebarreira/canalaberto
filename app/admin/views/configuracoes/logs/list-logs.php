		...
		<table id="listagem" class="ark-table --zebra --border --lighthead --hover" width="100%" cellpadding="0">
			<thead>
				<tr>
					...
					<th><a href="?rt=logs&amp;<?php echo $itens[$i]['link']?>"><?php echo $itens[$i]['tit'] . $itens[$i]['img'];?></a></th>
					<?php }?>
				</tr>
			</thead>
			<tbody>
				...
				<tr>
					<td><?php echo $loglist->get($i)->log_entidade;?></td>
					<td style="width: 525px;"><?php echo htmlentities(utf8_decode($loglist->get($i)->log_operacao));?></td>
					<td><?php echo $loglist->get($i)->log_entidadeLogin;?></td>
					<td><?php echo $loglist->get($i)->adm_nome;?></td>
					<td><?php echo $loglist->get($i)->log_ip;?></td>
					<td><?php echo $loglist->get($i)->log_data;?></td>
				</tr>
				<?}	?>				
			</tbody>
		</table>
