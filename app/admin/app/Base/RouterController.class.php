...
/*
	...
	Releases:
		...
		- 16-08-2023		 : load() - METHOD REQUEST validation (optional)
...
class RouterController
{
	...
	private function loader()
	{
		...
		foreach ($controller as $key => $value) {
			...
			$isAllowed = true;
			$allowMethods = (is_array(@$value["methods"])) ? $value["methods"] : array();

			// filter by method
			if (sizeof($allowMethods) > 0) {
				...
				foreach ($getController as $keyGet => $valueGet) {
					...
					if ($this->getUri() != "/") {
						if ((count($uri) - 2) == count($valueGet)) {
							array_push($arrCan, array("controller" => $value["controller"], "url" => $value["url"], "get" => $valueGet, "methods" => $value["methods"]));
						}
					} else {
		...
		foreach ($arrCan as $key => $value) {
			...
			$boolCan = array("controller" => $value["controller"], "url" => $value["url"], "get" => $value["get"], "methods" => $value["methods"]);
			$match   = $value["get"];
