...
CalendarConstructor.prototype.createMonthNameWrap = function(monthData) {
	var div = this.createNewElement("div", "calendar-header");
	var span = this.createNewElement("div", "month");
...
CalendarConstructor.prototype.createMonthTableHead = function() {
	var thead = this.createNewElement("thead", "calendar-th");
		...
		while ( day <= dayCount ) {
			...
			for (var i = 0; i < 7; i++) {
				if (monthData.weekDay(day) == i) {
					weekRow.innerHTML += "<td data-dia=\"" + day + "\">" + day + "</td>";
					day++;
				} else {
...
CalendarConstructor.prototype.createMonthTableWrap = function(monthData) {
    ...
    div.appendChild(this.createNewElement("div", "ring-left"));
    div.appendChild(this.createNewElement("div", "ring-right"));
	return div;	
}
...
CalendarConstructor.prototype.parseInputData = function(id, count, month, year) {
  this.startMonth = month > 12 || month === undefined ? this.currentMonth : month;
