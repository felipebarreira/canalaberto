 ...
 // $upload_folder = str_replace("/","\\",$upload_folder);
  ...
  if (in_array($ext,$ext_whitelist) && in_array($_FILES["uploaded_file"]["type"],$mime_whitelist) && 
    ...
    //path to upload dir
	  ...
	  $newname = $upload_folder.$filen.".".$ext; //editado by coresphere
		...
		$scale=1;
			...
			$tnImage = UnsharpMask($tnImage, 80, 0.5, 3); 
				...
				switch($ext) {
				  ...
				  imagepng($tnImage, $upload_folder.$filen.$tsetting["suffix"].".png"); //editado by coresphere
				  break;
 				  case "gif" :
				  //imagegif($tnImage, dirname(__FILE__).$upload_folder."\\".$filen.$tsetting["suffix"].".gif");
				  imagegif($tnImage, $upload_folder.$filen.$tsetting["suffix"].".gif"); //editado by coresphere
				  break;
				  default :
				  //imagejpeg($tnImage, dirname(__FILE__).$upload_folder."\\".$filen.$tsetting["suffix"].".".$ext);
				  imagejpeg($tnImage, $upload_folder.$filen.$tsetting["suffix"].".".$ext); //editado by coresphere
				  break;
				}
